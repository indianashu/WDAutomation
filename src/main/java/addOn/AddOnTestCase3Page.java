...
public class AddOnTestCase3Page extends BasePage {
		...
		public AddOnTestCase3Page(WebDriver webdriver) {
			super(webdriver);
		}

		Logger log = Logger.getLogger(AddOnTestCase3Page.class);
		...
		public void subscriptionOfPlan() throws IOException {
			...
			activeSince.clear();
			activeSince.sendKeys(BasePage.getCellData(xlsxName, sheetName, 13, i));

			WebElement activeUntil = driver.findElement(By.xpath("//input[@name='activeUntil']"));
			activeUntil.clear();
			activeUntil.sendKeys(BasePage.getCellData(xlsxName, sheetName, 14, i));
			...
			verifyConfirmationMsg("Saved Subscription Order Successfully For AddOnCustomer3");
		...
		public void consumptionOfPlan() throws IOException {
			...
			while(i <= 1){
				...
				WebElement activeSince = driver.findElement(By.xpath("//input[@name='activeSince']"));
				activeSince.clear();
				activeSince.sendKeys(BasePage.getCellData(xlsxName, sheetName, 15, i));
				...
				verifyConfirmationMsg("Saved Consumption Order Successfully For AddOnCustomer3 ");
